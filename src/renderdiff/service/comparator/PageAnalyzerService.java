...
//        }
    ...
    private void compareCodeState() {
        InputStream currentDoc = TextFileService.createInputStreamFromFile(currentPath + File.separator + urlHash + ".html");
        InputStream knownDoc = TextFileService.createInputStreamFromFile(basePath + knownState + File.separator + urlHash + ".html");
        ...
        if (currentDoc == null || knownDoc == null) {
            return;
        }

        String diff = HtmlComparatorService.diff(knownDoc, currentDoc);
        System.out.println("Matches known state: " + (diff.length() == 0));

        if (diff.length() > 0) {
            TextFileService.saveStringToFile(diff, currentPath + File.separator + urlHash + "-delta.html");
        }
...
//            new FileWriter( fileIn ).append( currentDoc ).close();
//            new FileWriter( fileOut ).append( knownDoc ).close();
//
//            String[] daisyDiffArgs = {
//                fileIn.getAbsolutePath(),
//                fileOut.getAbsolutePath(),
//                "-–file=" + fileDiff.getAbsolutePath()
//            };
